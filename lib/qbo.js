...
const DEFAULT_QUICKBOOKS_REDIRECT_URI='https://arborwise-os.vercel.app/api/oauth/quickbooks/callback';
...
export function qboRedirectUri(){
  const value=String(process.env.QBO_REDIRECT_URI||DEFAULT_QUICKBOOKS_REDIRECT_URI).trim();
  if(!/^https:\/\//i.test(value)){
    const error=new Error('QBO_REDIRECT_URI must be a complete HTTPS URL');
    error.status=409;
    throw error;
  }
  return value;
...
export function qboAuthUrl(state){
  ...
  url.searchParams.set('redirect_uri',qboRedirectUri());
