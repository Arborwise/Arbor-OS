...
async function parseResponse(response){
  ...
  if(!response.ok){
    const error=new Error(`QuickBooks ${response.status}: ${text.slice(0,700)}`);
...
export async function qboCompanyRequest(path,{method='GET',body=null}={}){
  ...
  const url=`${apiHost()}/v3/company/${encodeURIComponent(realmId)}/${cleanPath}${joiner}minorversion=75`;
