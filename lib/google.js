...
function quotedSheetName(tab){return `'${String(tab||'').replace(/'/g,"''")}'`;}
async function sheetRowsWithAuth(auth,tab,range='A:AA'){
  ...
  const response=await sheets.spreadsheets.values.get({
    ...
    range:`${quotedSheetName(tab)}!${range}`,
...
export async function sheetBatch(requests){
  const auth=await googleClient();
  return Promise.allSettled(requests.map(request=>sheetRowsWithAuth(auth,request.name,request.range)));
}
...
export async function googleSyncData({includeGmail=true}={}){
  ...
  if(gmailResult.status==='fulfilled'){messages=gmailResult.value;gmailAvailable=true;}
  else warnings.push(`Gmail: ${gmailResult.reason?.message||'unavailable'}`);
  if(calendarResult.status==='fulfilled'){events=calendarResult.value;calendarAvailable=true;}
  else warnings.push(`Calendar: ${calendarResult.reason?.message||'unavailable'}`);

  return {master,jobs,today,messages,events,warnings,gmailAvailable,calendarAvailable};
}
