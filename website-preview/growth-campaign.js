(() => {
  ...
  const pushEvent = (channel, detail = '') => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem('arborwise_lead_clicks') || '[]');
      stored.push(event);
      localStorage.setItem('arborwise_lead_clicks', JSON.stringify(stored.slice(-50)));
    } catch (_) {}
  };
  ...
  const decorateSms = link => {
    ...
    const clean = existing.replace(/^\[[^\]]+ WEBSITE LEAD\]\s*/i, '');
    link.href = `sms:${phone}?body=${encodeURIComponent(`[${sourceLabel}] ${clean}`.trim())}`;
  ...
  const decorateEmail = link => {
    ...
    const subject = params.get('subject') || `${city} Tree Question`;
    params.set('subject', `[${sourceLabel}] ${subject.replace(/^\[[^\]]+ WEBSITE LEAD\]\s*/i, '')}`);
    link.href = `mailto:${address}?${params.toString()}`;
  ...
  document.querySelectorAll('a[href^="/"]').forEach(link => {
    ...
    url.searchParams.set('utm_source', `${code.toLowerCase()}_side_site`);
    url.searchParams.set('utm_medium', 'local_page');
  ...
  if (cta && !document.querySelector('.fast-estimate')) {
    ...
    section.innerHTML = `
      <div class="fast-estimate-copy">
        ...
        <p>Complete the short form and your phone will open a text already labeled <strong>${sourceLabel}</strong>. Add three photos: the whole tree, the concern, and the trunk base.</p>
      </div>
    ...
    form.addEventListener('submit', event => {
      ...
      pushEvent('text_form', data.get('concern'));
  ...
  style.textContent = `
    ...
    .fast-estimate-form{display:grid;grid-template-columns:1fr 1fr;gap:13px}
