(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .aw-annie-logo-home.is-peeking img{animation:aw-home-peek 1.15s ease-in-out both}
      .aw-annie-logo-home.is-gone{opacity:0;transform:scale(.72);visibility:hidden}
      ...
      @keyframes aw-home-peek{
        0%{opacity:0;transform:translateY(16px) scale(.72) rotate(0)}
        34%{opacity:1;transform:translateY(2px) scale(.96) rotate(-5deg)}
        60%{transform:translateY(0) scale(1) rotate(5deg)}
        78%{transform:translateY(0) scale(1,.9) rotate(0)}
      ...
      @media(max-width:700px){
        .aw-annie-logo-home{right:7px;bottom:5px;width:59px;height:59px;border-width:2px}
        .aw-annie-logo-home img{width:51px;height:51px}
        .aw-annie-logo-home::after{bottom:-8px;font-size:.43rem;padding:3px 6px}
      ...
      @media(prefers-reduced-motion:reduce){
        ...
        .aw-annie-scroll__owl,
        .aw-annie-logo-home img{animation:none!important}
      }
    `;
  ...
  function buildLaunchpad() {
    ...
    if (home) return home;
    const brand = document.querySelector('.aw-brand') || document.querySelector('.brand') || document.querySelector('.site-header');
  ...
  function prepareDestination(index, nextSide) {
    const guide = getGuide();
    guide.classList.remove('is-left','is-right','is-settling');
  ...
  async function launchFromLogo() {
    if (launched || launching || flight) return;
    ...
    await animateOwlFrom(startRect, 1700);
    side = 'right';
    landedIndex = 0;
    tipIndex = 0;
    launched = true;
  ...
  async function flyAndLand(index) {
    ...
    const owl = getGuide().querySelector('.aw-annie-scroll__owl');
    if (!owl) return;
    ...
    side = nextSide;
    tipIndex = 0;
    await animateOwlFrom(startRect,1500);
    landedIndex = index;
