(() => {
  ...
  const setExactLogo = () => {
    ...
    exactLogoPromise.then(showLogo).catch(error => console.error(error));
  };
  ...
  const setAnnie = () => {
    ...
    fetch('assets/annie-correct.b64', { cache: 'no-store' })
      ...
      .then(encoded => {
        const source = `data:image/webp;base64,${encoded.replace(/\s+/g, '')}`;
        images.forEach(image => (image.src = source));
      })
      .catch(() => {});
  };

  const addClimberCircle = heroMedia => {
    if (!heroMedia) return;
    heroMedia.querySelectorAll('.climber-highlight').forEach(node => node.remove());
    const circle = document.createElement('span');
    circle.className = 'climber-highlight';
    circle.setAttribute('aria-hidden', 'true');
    heroMedia.appendChild(circle);
  };
  ...
  const applyHomepage = () => {
    ...
    if (actions) {
      actions.innerHTML =
        '<a class="button" href="#estimate">Get a Free Estimate</a><a class="button ghost" href="#concerns">What Is Wrong With My Tree?</a>';
    }
    ...
    if (heroCopy && !heroCopy.querySelector('.hero-annie')) {
      ...
      annie.innerHTML =
        '<img data-annie alt="Annie, the Arborwise owl with a red A on her chest"><p><strong>Annie says:</strong> A whole-tree photo helps us see the pattern before we arrive.</p>';
    ...
    if (heroCopy && !heroCopy.querySelector('.final-hero-promise')) {
      ...
      promise.innerHTML =
        '<strong>Nurture Your Nature</strong><span>Honest answers. Skilled work. Every recommendation has a reason.</span>';
  ...
  style.textContent = `
    ...
    .brand-main{max-width:1180px!important;grid-template-columns:120px minmax(360px,620px) 120px!important;gap:22px!important;align-items:center!important;justify-content:center!important}
    .brand-main .brand{width:100%!important;max-width:620px!important;justify-self:center!important}
    .brand-main .brand img{width:100%!important;height:auto!important;max-height:205px!important;object-fit:contain!important;display:block!important;filter:drop-shadow(0 10px 15px rgba(11,63,47,.15))!important}
    .header-contact{width:112px!important;min-height:58px!important;padding:6px 7px!important;border-radius:14px!important;border-bottom:0!important;gap:6px!important;box-shadow:0 5px 13px rgba(6,40,31,.10)!important}
    .header-contact .ui-icon{width:21px!important;height:21px!important}
    ...
    .desktop-contact small{font-size:.54rem!important}
    .desktop-contact strong{font-size:.72rem!important}
    ...
    .hero-copy h1 sup{font-family:Arial,sans-serif!important;font-size:.23em!important;line-height:1!important;vertical-align:super!important;margin-left:.12em!important;letter-spacing:.01em!important}
    ...
    .hero-media{position:relative!important;margin:0!important;background:#0b3f2f!important;min-height:0!important;overflow:hidden!important}
    .hero-media>img{width:100%!important;height:auto!important;min-height:0!important;object-fit:contain!important;object-position:center!important;display:block!important}
    .hero-media figcaption{z-index:5!important}
    ...
    .hero-annie{margin:24px auto 0!important;padding:12px 18px 12px 12px!important;border-radius:22px!important;background:linear-gradient(135deg,#edf9bd,#fffdf8)!important;border:2px solid #c9972f!important;display:grid!important;grid-template-columns:92px 1fr!important;gap:12px!important;align-items:center!important;text-align:left!important;box-shadow:0 10px 22px rgba(6,40,31,.10)!important}
    .hero-annie img{width:92px!important;height:92px!important;object-fit:contain!important;filter:drop-shadow(0 8px 8px rgba(6,40,31,.18))!important}
    .hero-annie p{margin:0!important;color:#244b3b!important;font-weight:700!important;line-height:1.35!important}
    .final-hero-promise{margin:24px auto 0!important;padding:17px 20px!important;border-top:2px solid var(--gold)!important;border-bottom:2px solid var(--gold)!important;display:grid!important;gap:3px!important;text-align:center!important}
    ...
    @media(max-width:760px){
      ...
      .hero-media{display:block!important;order:1!important;width:100%!important;min-height:0!important;border-radius:0!important}
      .hero-media>img{width:100%!important;height:auto!important;object-fit:contain!important;display:block!important}
      .hero-media figcaption{left:14px!important;right:auto!important;bottom:14px!important;max-width:calc(100% - 28px)!important;font-size:.78rem!important;padding:9px 13px!important}
      ...
      .hero-annie{grid-template-columns:76px 1fr!important;padding:10px 12px 10px 8px!important}
      .hero-annie img{width:76px!important;height:76px!important}
