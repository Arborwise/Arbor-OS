...
(() => {
  ...
  const BOOT_SYNC_KEY='arborwise-live-boot-sync-v44';
  ...
  function mergeAuthority(records){const current=authority();if(!current)return records;const byId=new Map((records||[]).map(item=>[normalizeId(item.id),item]));for(const record of current.records)byId.set(normalizeId(record.id),{...(byId.get(normalizeId(record.id))||{}),...record});return [...byId.values()];}
  function applyAuthorityToLocalState(){const current=authority();if(!current)return false;const state=safeParse(localStorage.getItem(STORAGE_KEY),{})||{};state.records=mergeAuthority(Array.isArray(state.records)?state.records:[]);state.currentOperationsUpdatedAt=current.updatedAt;state.currentSnapshotVersion=44;state.live=false;state.lastSync=current.updatedAt;localStorage.setItem(STORAGE_KEY,JSON.stringify(state));lastSync=current.updatedAt;live=false;statusMode='current';return true;}
  ...
  function liveLabel(){if(statusMode==='current')return `CURRENT OPERATIONS • UPDATED ${lastSync?new Date(lastSync).toLocaleString():''}`;if(statusMode==='locked')return 'SHARED DATA LOCKED • TAP REFRESH TO LOGIN';if(statusMode==='database')return 'DATABASE NOT CONNECTED • OFFLINE SNAPSHOT';return live?`LIVE SHARED DATA${lastSync?` • ${new Date(lastSync).toLocaleString()}`:''}`:'OFFLINE SNAPSHOT • TAP REFRESH';}
  ...
  function mapState(data){
    ...
    const records=mergeAuthority(remote.length?remote:(current.records||[]));
    ...
    const syncedAt=currentAuthority?.updatedAt||data.lastSync?.finished_at||data.lastSync?.started_at||null;
    const next={...current,records,hours,mileage,notes,live:!currentAuthority,lastSync:syncedAt,currentOperationsUpdatedAt:currentAuthority?.updatedAt||current.currentOperationsUpdatedAt||null};
    localStorage.setItem(STORAGE_KEY,JSON.stringify(next));
    lastSync=syncedAt;live=!currentAuthority;statusMode=currentAuthority?'current':'live';
  ...
  async function loadShared(showError=false,reload=true){
    ...
    try{const data=await api('/api/data');if(data.localMode){live=false;statusMode='database';setStatus(liveLabel());if(showError)toast('Shared database is not configured');return false;}const fingerprint=mapState(data);if(reload&&reloadFor(fingerprint))return true;setStatus(liveLabel());return true;}
  ...
  async function sync(silent=false,forceRemote=false){
    refresh.disabled=true;refresh.classList.add('spinning');if(!silent)toast('Refreshing current operations...');
    try{
      if(authority()&&!forceRemote){applyAuthorityToLocalState();setStatus(liveLabel());if(!silent)toast('Current operations loaded');setTimeout(()=>location.reload(),120);return;}
      let info;try{info=await connectionInfo();}catch(error){if(error.status===401){if(!silent)login();return;}throw error;}
      if(!info.quickbooks?.authorized&&!info.google?.authorized){if(!silent)connections(info);return;}
      const result=await api('/api/sync',{method:'POST'});localStorage.setItem(AUTO_SYNC_KEY,String(Date.now()));const data=await api('/api/data');const fingerprint=mapState(data);sessionStorage.setItem(FINGERPRINT_KEY,fingerprint);if(!silent){const qb=result.summary?.quickbooks||{},google=result.summary?.google||{};toast(`Synced • ${qb.estimates||0} estimates • ${qb.invoices||0} invoices • ${google.sheetRecords||0} sheet records • ${google.calendarRecords||0} schedule records`);}location.reload();
    }catch(error){if(!silent)toast(error.message);}finally{refresh.disabled=false;refresh.classList.remove('spinning');}
  }
  async function autoSync(){if(authority())return;const last=Number(localStorage.getItem(AUTO_SYNC_KEY)||0);if(!navigator.onLine||Date.now()-last<AUTO_SYNC_INTERVAL)return;await sync(true);}

  function value(id){return String($(id)?.value||'').trim();}
  function recordPayload(closedValue){const id=value('r-id')||`${value('r-type')==='job'?'WO':'EST'}-${Date.now()}`;return {id,type:value('r-type')||'est',name:value('r-name'),addr:value('r-address'),phone:value('r-phone'),email:value('r-email'),service:value('r-service'),money:Number(value('r-amount'))||0,category:value('r-category')||'RESIDENTIAL',who:value('r-who'),status:value('r-status'),date:value('r-work')||null,fuDate:value('r-follow')||null,time:value('r-time'),notes:value('r-notes'),closed:Boolean(closedValue),sharedVersion:44};}
  ...
  async function boot(){if(authority()){applyAuthorityToLocalState();setStatus(liveLabel());setTimeout(enforceStatus,50);return;}await loadShared(false,true);if(!sessionStorage.getItem(BOOT_SYNC_KEY)){sessionStorage.setItem(BOOT_SYNC_KEY,'1');await sync(true);}setTimeout(enforceStatus,50);}
  boot();
